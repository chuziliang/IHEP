...
int main(int argc,char** argv) {
    ...
    for(int fileNum = 0; fileNum<TotFileNum; fileNum++)
    {
        ...
        timenumber = runName[fileNum].substr(startPos + 1, endPos - startPos - 1);
        GCUnumber = runName[fileNum].substr(startPos - 6, 6);
        SubRunName = timenumber;// + GCUnumber;

        //TFile *ofile = new TFile((ResultPath+SubRunName+markedF+".root").c_str(),"recreate");
        TFile *ofile = new TFile((ResultPath+SubRunName+".root").c_str(),"recreate");
        ...
        {
            int run_nb = atoi(runName[fileNum].substr(4,3).c_str() );
            
            //string FileName = "/raw_data.bin"; ///all_raw_data.sci"; //
            //ifstream myFile ((rawDataPath+runName[fileNum]).c_str(), ios::in | ios::binary); //remove FileName for DAVIS
        ...
        c1->Print((ResultPath+SubRunName+".pdf(").c_str(),"pdf");
        c2->Print((ResultPath+SubRunName+".pdf").c_str(),"pdf");
        ...
        ofile->cd("canvas");
        c5->Write();
        c5->Print((ResultPath+SubRunName+".pdf").c_str(),"pdf");
        ...
        ofile->cd("canvas");
        c6->Write();
        c6->Print((ResultPath+SubRunName+".pdf").c_str(),"pdf");
        ...
        ofile->cd("canvas");
        c7->Write();
        c7->Print((ResultPath+SubRunName+".pdf").c_str(),"pdf");
        ...
        ofile->cd("canvas");
        c8->Write();
        c8->Print((ResultPath+SubRunName+".pdf").c_str(),"pdf");
        ...
        for (int iSign = 0; iSign<=1; iSign++) {    // ping:0, pong:1
            for (int iChn=0; iChn<128; iChn++) {
                ...
                if (iChn%16 == 15)
                {
                    c3->Print((ResultPath+SubRunName+".pdf").c_str(),"pdf");
                    ofile->cd();
                    ofile->cd("canvas");
                    c3->Write();
                }
