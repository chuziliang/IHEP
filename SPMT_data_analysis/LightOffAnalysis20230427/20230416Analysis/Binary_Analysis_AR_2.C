...
int main(int argc,char** argv)
{
	...
	ifstream file_ref_1;
  	file_ref_1.open("/junofs/users/chuziliang125/LightOffAnalysis20230427/20230416Analysis/input_binary_filename.txt");
	string FileDirectory = "/junofs/users/chuziliang125/LightOffAnalysis20230427/20230416Analysis/binary/";
	
	string filename;
	string OutputDirectory;
	string runName[400];
	string file_number;
	OutputDirectory="/junofs/users/chuziliang125/LightOffAnalysis20230427/20230416Analysis/result/";
	...
	for (int i = 1; i < argc; i++)
	{
		...
		{
			file_number = argv[i + 1];
			cout<<"This file number = "<<file_number<<endl;
			break;
		}
	}
	...
	//while (!file_ref_1.eof())
	{
		int TotFileNum=0;
		while (!file_ref_1.eof()) {
			file_ref_1>>runName[TotFileNum];
			TotFileNum++;
		}
		cout<<"Total file number : "<<TotFileNum<<endl;
		int fileNum = stoi(file_number);

		string line = FileDirectory + runName[fileNum];
	...
	/*
		...
		name_pdf = OutputDirectory+"pdf/"+token + "_Trigger_width.png";
	...
	/*
		...
		for(int i=0;i<128; i++)
		{
			...
			if (compteur_plots == 8 || i == 127)
			{
				...
				if (compteur_canvas == 1)
				{
					name_pdf = OutputDirectory+"pdf/"+token + "_Charge_pong.pdf(";
					Histo_Charge_pong_HG_pdf->Print(name_pdf,"pdf");
				}
				else if (i==127)
				{
	...
	*/
		...
		for(int i=0;i<128; i++)
		{
			...
			if (compteur_plots == 8 || i == 127)
			{
				...
				if (compteur_canvas == 1)
				{
					name_pdf = OutputDirectory+"pdf/"+token + "_Trigger_falling_time.pdf(";
		...
		*/
		...
		// string name = OutputDirectory+"pdf/"+token + "_Charge_trigger_width.png";
