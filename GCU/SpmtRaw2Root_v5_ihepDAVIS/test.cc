...
int main(int argc,char** argv) {
    ...
    //if (! DrawPdf)
    {
        ...
        for (int iSign = 0; iSign<=1; iSign++) {    // ping:0, pong:1
            for (int iChn=0; iChn<128; iChn++) {
                if ( iSign == 0 ) c1->cd((iChn/16)+1);
                if ( iSign == 1 ) c2->cd((iChn/16)+1);
                h_Adc_H[iSign][iChn] -> Draw("same");
                if (((iChn%16)+1) < 9)
                {
                    h_Adc_H[iSign][iChn]->SetLineColor(((iChn%16)+1)+1);
                }
                if (((iChn%16)+1) >=9)
                {
                    h_Adc_H[iSign][iChn]->SetLineColor((((iChn%16)+1)-7)*10);
                }
            }
        }
        c1->Print("plotsc1.pdf","pdf");
        c2->Print("plotsc2.pdf","pdf");
        TCanvas *c3;
        c3 = new TCanvas("c3");
        c3->Divide(4,2);
        TCanvas *c4;
        ...
        for (int iSign = 0; iSign<=1; iSign++) {
            for (int iChn=0; iChn<128; iChn++) {
                if ( iSign == 0 ) c3->cd((iChn/16)+1);
                if ( iSign == 1 ) c4->cd((iChn/16)+1);
                h_Adc_L[iSign][iChn] -> Draw("same");
                if (((iChn%16)+1) < 9)
                {
                    h_Adc_L[iSign][iChn]->SetLineColor(((iChn%16)+1)+1);
                }
                if (((iChn%16)+1) >=9)
                {
                    h_Adc_L[iSign][iChn]->SetLineColor((((iChn%16)+1)-7)*10);
                }
            }
        }
        c3->Print("plotsc3.pdf","pdf");
        c4->Print("plotsc4.pdf","pdf");
    }
    ofile->cd();
    ofile->Write();
    aEvent->Write();
