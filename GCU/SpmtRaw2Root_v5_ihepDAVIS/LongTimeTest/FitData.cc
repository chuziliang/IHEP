...
int main(int argc,char** argv) {
    ...
    cout<<fileNum<<"\t"<<i<<" "<<runName[fileNum-1]<<endl;
        ...
        ifstream FitPedestal;
        FitPedestal.open("FitPedestalResult.txt",ios::ate);
        FitPedestal.seekg(-2,FitPedestal.cur);
        while(FitPedestal.peek() != FitPedestal.widen('\n'))
        {
            FitPedestal.seekg(-1,FitPedestal.cur);
        }
        ofstream FitData;
        FitData.open("FitDataResult.txt",ios::app);
        FitData<<fileNum<<"\t"<<i<<" "<<runName[fileNum-1]<<endl;

        TFile *ofile = new TFile((markedF+".root").c_str(),"recreate");
    ...
    for (int lpFile=0; lpFile<fileNum; lpFile++) {
        ...
        for (int iSign = 0; iSign<=1; iSign++) {    // ping:0, pong:1
            for (int iChn=0; iChn<128; iChn++) {
                ...
                double PedestalTemp, nan;
                FitPedestal >> PedestalTemp >> nan;
                func[iSign][iChn] = new TF1("SpecModel","gaus",PedestalTemp+20,PedestalTemp+180);
                double para[3];
                h_Adc_H[iSign][iChn]->Fit(func[iSign][iChn], "R");
                func[iSign][iChn]->GetParameters(para);
	        	gStyle->SetOptFit(1011);
                
                double gain = (para[1] - PedestalTemp)*0.049;//这里需要pedestal
                double resulotion = 100.0*para[2]/(para[1]-PedestalTemp);
                double DarkRate = h_Adc_H[iSign][iChn]->GetEntries()/600.0;
                FitData<<gain<<"\t"<<resulotion<<"\t"<<DarkRate<<"\t";
                if (iSign == 0 && iChn == 7)
                {
                   
                   c1->Print((runName[fileNum-1]+".pdf(").c_str(),"pdf");
                }
                else if (iChn%8 == 7)
                {
                    c1->Print((runName[fileNum-1]+".pdf").c_str(),"pdf");
