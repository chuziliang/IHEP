...
int GetChargeSpectrumDetSim_NoBar(const int FileID)
{ 
    TString FileName = TString::Format("root://junoeos01.ihep.ac.cn//eos/juno/groups/Calibration/chuziliang125/ShadowingEffect/NoBar/root/detsim_%1d.root",FileID);
    TFile *inputfile = TFile::Open(FileName,"read");
    if (!inputfile) return 0;
    
    TString outputfilename = TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/FindNode/ShadowingEffect/DetSimChargeSpectrum/NoBar/ChargeSpectrum%1d.root",FileID);
