...
int Analysis(const int filenum)
{
    ...
    string filename = "root://junoeos01.ihep.ac.cn//eos/juno/groups/Calibration/chuziliang125/B12/root/detsim_"+to_string(filenum)+".root";
    ...
    // 输出文件
    string outputname;
    outputname = "/junofs/users/chuziliang125/LPMTChargeCalibration/B12/Analysis/ChargeSpectrum/Charge_"+to_string(filenum)+".root";
    ...
    for (int i = 0; i < nentries; i++)
    {
        ...
        for (int LID = 0; LID < 17612; LID++)
        {
            OutputLPMT = LPMTPE[LID]/LPMTQE[LID];
            LPMT_ID = LID;
            
            for (int id = 0; id < 6; id++)
            {
                SPMT_ID[id] = CorrespondingSPMT[LID][id];
                if (SPMT_ID[id] >= 300000)
                {
                    OutputSPMT[id] = SPMTPE[SPMT_ID[id]-300000]/SPMTQE[SPMT_ID[id]-300000];
                }
                else
                {
                    OutputSPMT[id] = -1;
