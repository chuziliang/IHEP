...
void npeLPMTtonpeSPMT_BinnedByR()
{
    double OutputLPMT; 
    double OutputSPMT[6];
    double OutputTheta;
        ...
        for(int RNum = 0; RNum < 50; RNum++)
        {
            ...
            for (int filenum = 0; filenum < 10; filenum++)
            {
                ...
                for (int i = 0; i < OutputTree->GetEntries(); i++)
                {
                    ...
                    EdepR[RNum]->Fill(sqrt(edep_x*edep_x + edep_y*edep_y + edep_z*edep_z));
    ...
    TFile *output = new TFile("/junofs/users/chuziliang125/LPMTChargeCalibration/B12/Analysis/npeLPMTtonpeSPMT_BinnedByR.root","RECREATE");
