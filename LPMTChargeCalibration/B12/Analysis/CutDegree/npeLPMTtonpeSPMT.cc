...
void npeLPMTtonpeSPMT( double CutDegree)
{
    double OutputLPMT; 
    double OutputSPMT[6];
    double OutputTheta;
        ...
        for(int RNum = 0; RNum < 50; RNum++)
        {
            ...
            for (int filenum = 0; filenum < 10; filenum++)
            {
                ...
                for (int i = 0; i < OutputTree->GetEntries(); i++)
                {
                    ...
                    if (OutputTheta<CutDegree && OutputTheta>(CutDegree-5))
                    {
    ...
    TString outputname = TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/B12/Analysis/CutDegree/npeLPMTtonpeSPMT_%.1fDEG.root",CutDegree);
