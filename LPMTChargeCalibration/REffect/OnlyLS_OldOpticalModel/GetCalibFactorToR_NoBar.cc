...
int GetCalibFactorToR_NoBar(const int CutLevel)
{
    ...
    for (int RID = 0; RID < NR; RID++)
    {
        ...
        {
            TString filename = TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/REffect/OnlyLS_OldOpticalModel/ShadowingEffect/R%1d_Theta%1d.root",RID,ThetaID);
    ...
    CalibFactorToR->GetXaxis()->SetTitle("R [m]");
    CalibFactorToR->GetYaxis()->SetTitle("Calibration factor");
    ...
    CalibFactorToR->GetXaxis()->SetTitleOffset(0.92);
    CalibFactorToR->GetYaxis()->SetTitleOffset(1.08);
    CalibFactorToR->GetYaxis()->SetRangeUser(44.5,46.1);

    c1 -> cd();
    TText *text = new TText(20,45.5,"Fluctuation about 1.3%");
    text->SetTextFont(22);
