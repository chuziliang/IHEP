...
int GetCalibFactorToR_NoBar(const int CutLevel)
{
    ...
    for (int RID = 0; RID < NR; RID++)
    {
        ...
        {
            TString filename = TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/REffect/OnlyLS/ShadowingEffect/R%1d_Theta%1d.root",RID,ThetaID);
    ...
    c1 -> cd();
    TText *text = new TText(20,51.4,"Fluctuation about 2%");
    text->SetTextFont(22);
