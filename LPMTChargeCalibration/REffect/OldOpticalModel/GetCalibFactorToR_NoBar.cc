...
int GetCalibFactorToR_NoBar(const int CutLevel)
{
            ...
            for (int DID = 0; DID < ND; DID++)
            {
                for (int PhiID = 0; PhiID < NPhi; PhiID++)
                {
                    ...
                    else if (CutLevel == 5)
                    {
                        if (RID == 0)
                        {
                            if ((ThetaID==0 && (DID<9)) || (ThetaID==1 && (DID<14)) || (ThetaID==2 && (DID<16)) || (ThetaID>2))
                            {
                                continue;
                            }
                        }
                        ...
                        {
                            if ((ThetaID==0 && (DID<5)) || (ThetaID==1 && (DID<14)) || (ThetaID==2 && (DID<16)) || (ThetaID>2))
                            {
                                continue;
                            }
                        }
    ...
    CalibFactorToR->Draw();
    CalibFactorToR->SetName("CalibFactorToR");
    ...
    TFile *outputfile = TFile::Open("/junofs/users/chuziliang125/LPMTChargeCalibration/REffect/OldOpticalModel/CalibFactorToR_Graphs/Graph_NoCut.root","recreate");
    CalibFactorToR->Write();
    outputfile->Close();
