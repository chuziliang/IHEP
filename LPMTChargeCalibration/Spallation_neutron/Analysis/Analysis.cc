...
int Analysis(const int filenum)
{
    double LPMTQE[17612];
    double SPMTQE[25600];
    ...
    // 输出文件
    string outputname;
    outputname = "/junofs/users/chuziliang125/LPMTChargeCalibration/Spallation_neutron/Analysis/ChargeSpectrum/Charge_"+to_string(filenum)+".root";
    ...
    {
        OutputTree[RNum] = new TTree(TString::Format("Charge_R%1d",RNum),"LPMT and SPMT charge");
        OutputTree[RNum]->Branch("LPMTCharge_QE",&OutputLPMT,"LPMTCharge_QE/D");            
        OutputTree[RNum]->Branch("SPMTCharge_QE",OutputSPMT,"SPMTCharge_QE[6]/D");
        OutputTree[RNum]->Branch("LPMT_ID",&LPMT_ID,"LPMT_ID/I");
        OutputTree[RNum]->Branch("SPMT_ID",SPMT_ID,"SPMT_ID[6]/I");
        OutputTree[RNum]->Branch("Theta",&OutputTheta,"Theta/D");
        OutputTree[RNum]->Branch("Phi",&OutputPhi,"Phi/D");
        OutputTree[RNum]->Branch("Distance between LPMT and event",&OutputR,"R/D");
        OutputTree[RNum]->Branch("edep_x",&edep_x,"edep_x/D");
        OutputTree[RNum]->Branch("edep_y",&edep_y,"edep_y/D");
        OutputTree[RNum]->Branch("edep_z",&edep_z,"edep_z/D");
        OutputTree[RNum]->Branch("edep",&edepEnergy,"edep/D");
    }
