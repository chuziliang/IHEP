...
void DrawNonlinearity_RecToTrue()
{
    ...
    for (int i = 0; i < 21; i++)
    {
        ...
        DataWithError RoughP0Charge_Nonlinearity = Total(RoughP0Charge);
        DataWithError TrueP0Charge_Nonlinearity = Total(TrueP0Charge);
        DataWithError nPE_Nonlinearity = Total(nPE);
        DataWithError TruenPE_Nonlinearity = Total(TruenPE);
    ...
    TF1 *LinearityLine = new TF1("LinearityLine","x",0,3);
    ...
    NPENonlinearity_Total->SetMarkerStyle(21);
    NPENonlinearity_Total->SetLineWidth(3);
    NPENonlinearity_Total->SetTitle("");
    NPENonlinearity_Total->GetXaxis()->SetTitle("#mu^{i}_{S} [p.e.]");
    NPENonlinearity_Total->GetYaxis()->SetTitle("#alpha^{CH}_{S}(#mu^{i}_{S})#upoint #mu^{i}_{S} [p.e.]");
    // NPENonlinearity_Total->GetXaxis()->CenterTitle();
    // NPENonlinearity_Total->GetYaxis()->CenterTitle();
    NPENonlinearity_Total->GetXaxis()->SetLimits(0,3);
    NPENonlinearity_Total->GetYaxis()->SetRangeUser(0,3);
    NPENonlinearity_Total->GetXaxis()->SetRangeUser(0,3);

    // NPENonlinearity_Total->Draw("a");
    NPENonlinearity_Total->Draw("a P");

    TLegend *legendb =new TLegend(0.6,0.65,0.88,0.85);
    legendb->SetTextFont(22);
    // legendb->SetTextSize(0.04);
    legendb->AddEntry(NPENonlinearity_Total,"Charge method","p");
    ...
    ReviseNPENonlinearity_Total->SetMarkerStyle(21);
    ReviseNPENonlinearity_Total->SetLineWidth(3);

    // ReviseNPENonlinearity_Total->Draw("same P");

    // legendb->AddEntry(ReviseNPENonlinearity_Total,"Charge method w/o DCR effect","p");
    ...
    RoughP0Nonlinearity_Total->SetMarkerStyle(22);
    RoughP0Nonlinearity_Total->SetLineWidth(3);

    RoughP0Nonlinearity_Total->Draw("same P");

    legendb->AddEntry(RoughP0Nonlinearity_Total,"Occupancy method before correction","p");
    ...
    ReviseP0Nonlinearity_Total->Draw("same P");
    LinearityLine->SetLineColor(1);
    LinearityLine->SetLineStyle(2);
    LinearityLine->Draw("same");

    legendb->AddEntry(ReviseP0Nonlinearity_Total,"Occupancy method after correction","p");
