...
void DrawNonlinearityCompare_cut_SPMTonly_revises_NoDCR()
{
    GetP0();
    ...
    DataWithError DarkRateData;
    DarkRateData.Data = 0;
    DarkRateData.Error = 0;

    TFile *inputfile_P0 = TFile::Open("/junofs/users/chuziliang125/LPMTChargeCalibration/LaserAnalysis/WaveformReconstruction/DrawNonlinearity/AllSPMT/jobs/NoDCR/P0Charge_NoDCR.root","read");
    ...
    // P0Charge->SetBranchAddress("TrueSPMTMeanCharge",DetSimMeanCharge);
    // P0Charge->SetBranchAddress("ErrorTrueSPMTMeanCharge",DetSimErrorCharge);
    ...
    TH1 *TrueP0Hist[2];
    TH1 *NonlinearityHist[2];
    for (int i = 0; i < 2; i++)
    {
        TrueP0Hist[i] = new TH1F(TString::Format("TruePE hist %1d",i),TString::Format("TruePE hist %1d",i),1000,0,4);
        NonlinearityHist[i] = new TH1F(TString::Format("Nonlinearity hist %1d",i),TString::Format("Nonlinearity hist %1d",i),100,-0.2,0.2);
    }
    ...
    for (int i = 0; i < 21; i++)
    {
        ...
        for (int PMTID = 0; PMTID < 25600; PMTID++)
        {
            ...
            TrueP0Charge[PMTID] = GetTrueCharge(RoughP0Charge[PMTID],DarkRateData);
            // if (isnan(TrueP0Charge[PMTID].Error))
            // {
            //     cout<<"PMTID = "<<PMTID<<endl;
            //     cout<<"RC = "<<RoughP0Charge[PMTID].Data<<"  Error = "<<RoughP0Charge[PMTID].Error<<endl;
            //     cout<<"DetC = "<<DetSimMeanCharge[PMTID]<<"  Error = "<<DetSimErrorCharge[PMTID]<<endl;
            // }
            

            nPE[PMTID].Data = nPEMeanCharge[PMTID];
            nPE[PMTID].Error = nPEErrorCharge[PMTID];
            TruenPE[PMTID] = GetTrueCharge_ChargeMethod(nPE[PMTID],DarkRateData);
            DetSim[PMTID].Data = DetSimMeanCharge[PMTID];
            DetSim[PMTID].Error = DetSimErrorCharge[PMTID];
            if (i==0)
            {
                TrueP0Hist[0]->Fill(TrueP0Charge[PMTID].Data);
                if (DetSim[PMTID].Data > 0)
                {
                    NonlinearityHist[0]->Fill(TrueP0Charge[PMTID].Data/DetSim[PMTID].Data - 1);
                }
            }
            if (i==20)
            {
                TrueP0Hist[1]->Fill(TrueP0Charge[PMTID].Data);
                if (DetSim[PMTID].Data > 0)
                {
                    NonlinearityHist[1]->Fill(TrueP0Charge[PMTID].Data/DetSim[PMTID].Data - 1);
                }
            }
    ...
    for (int i = 0; i < 2; i++)
    {
        cout<<ReviseP0Nonlinearity_Total->GetErrorY(i)<<endl;
    }
    ...
    // legendb->AddEntry(NPENonlinearity_Total,"Charge method w/ DCR and threshold effect","lpe");
    ...
    // legendb->AddEntry(RoughP0Nonlinearity_Total,"Occupancy method w/ DCR and threshold effect","lpe");
    ...
    ReviseP0Nonlinearity_Total->SetTitle("Nonlinearity of different methods");
    ReviseP0Nonlinearity_Total->GetXaxis()->SetTitle("True SPMT charge form DetSim(p.e.)");
    ReviseP0Nonlinearity_Total->GetYaxis()->SetTitle("Nonlinearity");
    ReviseP0Nonlinearity_Total->Draw("a L P");
    ...
    NonlinearityHist[0]->Draw();
    TCanvas *c6 = new TCanvas("c6","c6",1);
    NonlinearityHist[1]->Draw();
}