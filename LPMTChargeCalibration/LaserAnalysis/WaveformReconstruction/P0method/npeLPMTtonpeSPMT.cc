void npeLPMTtonpeSPMT()
{
    ...
    for (int nPE = 1; nPE < 200; nPE++)
    {
        ...
        for (int id = 0; id < 6; id++)
        {
            if (SPMTHist[id][nPE]->GetBinContent(1) > 0 )
            {
                P0LargerThan0 = true;
            }
            else
        ...
        if (entries > 1 && SelectHist)
        {
            ...
            for (int id = 0; id < 6; id++)
            {
                double P0 = (double)SPMTHist[id][nPE]->GetBinContent(1)/SPMTHist[id][nPE]->GetEntries();
                SPMTMeanCharge[id][n] = -TMath::Log(P0);
                double miuP0 = P0*(1 - P0)/SPMTHist[id][nPE]->GetEntries();
                miuP0 = sqrt(miuP0);
