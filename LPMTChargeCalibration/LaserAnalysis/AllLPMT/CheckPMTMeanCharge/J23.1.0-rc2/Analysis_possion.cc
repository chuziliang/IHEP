...
void Analysis_possion()
{
    ...
    TFile *inputfile = TFile::Open("root://junoeos01.ihep.ac.cn//eos/juno/groups/Calibration/chuziliang125/test/detsim_0.root");
    if (!inputfile)
    {
    ...
    TFile *outputfile = TFile::Open("result_possion.root","recreate");

    TH1 *LPMTMeanQE[17612];
    TStatistic *SPMTMeanQE[25600];
    int ZeroNum[17612];

    for (int ID = 0; ID < 17612; ID++)
    {
        LPMTMeanQE[ID] = new TH1F(TString::Format("LPMT Mean_QE Chn%1d",ID),TString::Format("LPMT Mean_QE Chn%1d",ID),100,0,100);
    }
    ...
    for (int ID = 0; ID < 17612; ID++)
    {
        double P0 = (double)ZeroNum[ID]/nentries;
        double miu = -TMath::Log(P0);
        LPMTResult->Fill(miu/LPMTQE[ID]);
        // LPMTResult->Fill(LPMTMeanQE[ID]->GetMean());

        switch (LPMTType[ID])
        {
        case 1:
            LPMTResult_H->Fill(miu/LPMTQE[ID]);
            // LPMTResult_H->Fill(LPMTMeanQE[ID]->GetMean());
            LPMTQEHist_H->Fill(LPMTQE[ID]);
            break;

        case 2:
            LPMTResult_N->Fill(miu/LPMTQE[ID]);
            // LPMTResult_N->Fill(LPMTMeanQE[ID]->GetMean());
            LPMTQEHist_N->Fill(LPMTQE[ID]);
            break;
        
        case 3:
            LPMTResult_HQ->Fill(miu/LPMTQE[ID]);
