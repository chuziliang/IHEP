...
{
    ifstream file("/cvmfs/juno.ihep.ac.cn/centos7_amd64_gcc1120/Pre-Release/J23.1.0-rc8.dc1/data/Detector/Geometry/PMTPos_CD_LPMT.csv");
    string line;
...
double GetNodePosition(Vector3D (&NodePosition)[590]) {
    ...
    TString filename_anchor="/cvmfs/juno.ihep.ac.cn/centos7_amd64_gcc1120/Pre-Release/J23.1.0-rc8.dc1/junosw/Simulation/DetSimV2/DetSimOptions/data/Strut_Anchor_Acrylic.csv";
...
int GetLPMTType(int (&LPMTType)[17612])
{
    ...
    TString filename_LPMT="/cvmfs/juno.ihep.ac.cn/centos7_amd64_gcc1120/Pre-Release/J23.1.0-rc8.dc1/data/Detector/Geometry/PMTType_CD_LPMT.csv";
...
int npeLPMTtonpeSPMT_B12_DetSim_AllPMT(int ChosenType, int CutLevel, int FileId)
{
    // int MinFile = 0;
    // int MaxFile = 465;
    ...
    vector<int> SPMT0_Calib(21,0);
    vector<int> SPMT0_DetSim(21,0);
    ...
    // 定义结束
    // ----------------------------------------分割线--------------------------------------------------------------------------------------------
    // 开始读取文件，向TStatistic中填入数据

    cout<<"Start reading"<<endl;
    // for (int FileId = MinFile; FileId < MaxFile; FileId++)
    {
        TString InputFileName = TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/Spallation_neutron/OldOpticalModel/ChargeSpectrum/ChargeSpectrum%1d.root",FileId);
        ...
        vector<double> *CalibChargeSPMT = nullptr;
        vector<double> *TrueChargeLPMT = nullptr;
        vector<double> *TrueChargeSPMT = nullptr;
        double edep_x, edep_y, edep_z;
        double edep;

        charge->SetBranchAddress("LPMTCharge",&CalibChargeLPMT);
        charge->SetBranchAddress("SPMTCharge",&CalibChargeSPMT);
        ...
        for (int entry = 0; entry < charge->GetEntries(); entry++)
        {
            ...
            for (int LPMTID = 0; LPMTID < 17612; LPMTID++)
            {
                ...
                {
                    if ((Theta<=10 && DistanceToImage<2500) || (Theta<=20 && Theta>10 && DistanceToImage<2500) || (Theta<=40 && Theta>20 && DistanceToImage<3500))
                    {
                        continue;
                    }
                }
                ...
                {
                    if ((Theta<=10 && Phi<250 && Phi>100 && DistanceToImage<2500) || (Theta<=10 && (Phi>250 || Phi<100) && DistanceToImage<1500) || (Theta<=20 && Theta>10 && DistanceToImage<2500) || (Theta<=40 && Theta>20 && DistanceToImage<3500))
                    {
                        continue;
                    }
                }
                else if (CutLevel == 4)
                {
                    if (R < 10.9)
                    {
                        if ((Theta<10 && DistanceToImage<1800) || (Theta<20 && Theta>10 && DistanceToImage<2800) || (Theta<30 && Theta>20 && DistanceToImage<3200) || (Theta>30))
                        {
                            continue;
                        }
                    }
                    ...
                    {
                        if ((Theta<10 && DistanceToImage<1000) || (Theta<20 && Theta>10 && DistanceToImage<2800) || (Theta<30 && Theta>20 && DistanceToImage<3200) || (Theta>30))
                        {
                            continue;
                        }
                    }
                ...
                for (int ID = 0; ID < 6; ID++)
                {
                    if ((CorrespondingSPMT[LPMTID][ID] >= 300000))
                    {
                        ...
                        Sta_SPMTMu[QNum]->Fill((*TrueChargeSPMT)[CorrespondingSPMT[LPMTID][ID] - 300000]/SPMTQE[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        ...
                        Hist_SPMTMu_QE[QNum]->Fill((*TrueChargeSPMT)[CorrespondingSPMT[LPMTID][ID] - 300000]/SPMTQE[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        Hist_SPMTMu[QNum]->Fill((*TrueChargeSPMT)[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        if ((*TrueChargeSPMT)[CorrespondingSPMT[LPMTID][ID] - 300000]==0)
                        {
                            SPMT0_DetSim[QNum]++;
                        }
                        if ((*CalibChargeSPMT)[CorrespondingSPMT[LPMTID][ID] - 300000]==0)
                        {
                            SPMT0_Calib[QNum]++;
                        }
                    }
    ...
    TFile *outputfile = TFile::Open("/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/Spallation_neutron/OldOpticalModel/result/npeLPMTtonpeSPMT_B12_DetSim_AllPMT_"+ChosenTypeString+TString::Format("_File%1d",FileId)+"_Cut"+TString::Format("%1d",CutLevel)+".root","RECREATE");
    TTree *SPMT0_Tree = new TTree("SPMT0","SPMT0");
    int SPMT0_Calib_i;
    int SPMT0_DetSim_i;
    SPMT0_Tree->Branch("Calib",&SPMT0_Calib_i);
    SPMT0_Tree->Branch("DetSim",&SPMT0_DetSim_i);
    ...
    for (int QNum = 0; QNum < 21; QNum++)
    {
        ...
        SPMT0_Calib_i = SPMT0_Calib[QNum];
        SPMT0_DetSim_i = SPMT0_DetSim[QNum];
        SPMT0_Tree->Fill();
    }
    ...
    // TruenpeLPMTtonpeSPMT_tot->SetMarkerColor(4);
    // TruenpeLPMTtonpeSPMT_tot->SetLineColor(4);
    ...
    // TruenpeLPMTtonpeSPMT_tot->GetXaxis()->CenterTitle();
    // TruenpeLPMTtonpeSPMT_tot->GetYaxis()->CenterTitle();
    // TruenpeLPMTtonpeSPMT_tot->GetXaxis()->SetTitleFont(22);
    // TruenpeLPMTtonpeSPMT_tot->GetYaxis()->SetTitleFont(22);
    // TruenpeLPMTtonpeSPMT_tot->GetXaxis()->SetTitleSize(0.06);
    // TruenpeLPMTtonpeSPMT_tot->GetYaxis()->SetTitleSize(0.06);
    // TruenpeLPMTtonpeSPMT_tot->GetXaxis()->SetLabelSize(0.06);
    // TruenpeLPMTtonpeSPMT_tot->GetYaxis()->SetLabelSize(0.06);
    // TruenpeLPMTtonpeSPMT_tot->GetXaxis()->SetTitleOffset(0.83);
    // TruenpeLPMTtonpeSPMT_tot->GetYaxis()->SetTitleOffset(1.30);
