void MergeResult(int ChosenType, int CutLevel)
{
    ...
    for (int FileId = 0; FileId < 2; FileId++)
    {
        ...
        if (FileId == 0)
        {
            InputName = "/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/Alter_10Percent_100PE_B12_OldOpticalModel/result/npeLPMTtonpeSPMT_B12_DetSim_AllPMT_"+ChosenTypeString+"_Cut"+TString::Format("%1d",CutLevel)+".root";
        ...
        {
            InputName = "/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/Spallation_neutron/OldOpticalModel/MeragedResult/npeLPMTtonpeSPMT_B12_DetSim_AllPMT_"+ChosenTypeString+"_Cut"+TString::Format("%1d",CutLevel)+".root";
            TFile *inputfile = TFile::Open(InputName,"read");
            if (!inputfile) 
            {
                cout<<"File "<<FileId<<" not exist"<<endl;
            ...
            TTree *SPMT0_Tree = (TTree*)inputfile->Get("SPMT0");
            if (!SPMT0_Tree) continue;
            unsigned long long SPMT0_Calib_i;
            unsigned long long SPMT0_DetSim_i;
            SPMT0_Tree->SetBranchAddress("Calib",&SPMT0_Calib_i);
            SPMT0_Tree->SetBranchAddress("DetSim",&SPMT0_DetSim_i);
    ...
    TString OutputName = "/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/CombineB12AndNeutron/result/Combine_B12_Neutron_"+ChosenTypeString+"_Cut"+TString::Format("%1d",CutLevel)+".root";
