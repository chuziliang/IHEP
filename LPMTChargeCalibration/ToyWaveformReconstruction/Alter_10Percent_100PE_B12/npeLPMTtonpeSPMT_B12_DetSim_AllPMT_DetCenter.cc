...
int npeLPMTtonpeSPMT_B12_DetSim_AllPMT_DetCenter(int ChosenType)
{
    ...
    double DistanceToImage[17612];

    for (int LPMTID = 0; LPMTID < 17612; LPMTID++)
    {
        double Phi;
        GetDtoImage(DistanceToImage[LPMTID],Phi,LPMTPosition[LPMTID],ImagePoint,Edep_position);
    }
    


    cout<<"Start reading"<<endl;
    for (int FileId = 0; FileId < 21; FileId++)
    {
        TString InputFileName = TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/LaserAnalysis/AllLPMT/CombineChargeSpectrum/CombineCharge_%1dMeV.root",energy[FileId]);
        ...
        for (int entry = 0; entry < charge->GetEntries(); entry++)
        {
            ...
            for (int LPMTID = 0; LPMTID < 17612; LPMTID++)
            {
                ...
                if (DistanceToImage[LPMTID]<1000)
                {
                    continue;
                }
                ...
                Sta_TrueLPMTMu[FileId]->Fill(LPMTCharge[LPMTID]);
                Sta_TrueLPMTMu_QE[FileId]->Fill(LPMTCharge[LPMTID]/LPMTQE[LPMTID]);
                Hist_TrueLPMTMu_QE[FileId]->Fill(LPMTCharge[LPMTID]/LPMTQE[LPMTID]);
                Hist_TrueLPMTMu[FileId]->Fill(LPMTCharge[LPMTID]);
                // SPMT0中Fill选中的LPMT_Event对
                for (int ID = 0; ID < 6; ID++)
                {
                    if ((CorrespondingSPMT[LPMTID][ID] >= 300000))
                    {
                        Sta_SPMTDCR[FileId]->Fill(SPMTDCR[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        Sta_SPMTQE[FileId]->Fill(SPMTQE[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        Sta_LPMTQE[FileId]->Fill(LPMTQE[LPMTID]);
                        Sta_SPMTMu[FileId]->Fill(SPMTCharge[CorrespondingSPMT[LPMTID][ID] - 300000]/SPMTQE[CorrespondingSPMT[LPMTID][ID] - 300000]);

                        Hist_SPMTDCR[FileId]->Fill(SPMTDCR[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        Hist_SPMTQE[FileId]->Fill(SPMTQE[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        Hist_LPMTQE[FileId]->Fill(LPMTQE[LPMTID]);
                        Hist_SPMTMu_QE[FileId]->Fill(SPMTCharge[CorrespondingSPMT[LPMTID][ID] - 300000]/SPMTQE[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        Hist_SPMTMu[FileId]->Fill(SPMTCharge[CorrespondingSPMT[LPMTID][ID] - 300000]);
                        if (SPMTCharge[CorrespondingSPMT[LPMTID][ID] - 300000]==0)
                        {
                            SPMT0[FileId]++;
                        }
                    }
    ...
    for (int FileId = 0; FileId < 21; FileId++)
    {
        ...
        double TrueLPMTMu = Sta_TrueLPMTMu[FileId]->GetMean();
        double TrueLPMTMu_Error = Sta_TrueLPMTMu[FileId]->GetMeanErr();

        double TrueLPMTMu_QE = Sta_TrueLPMTMu_QE[FileId]->GetMean();
        double TrueLPMTMu_QE_Error = Sta_TrueLPMTMu_QE[FileId]->GetMeanErr();

        double AverageSPMTQE = Sta_SPMTQE[FileId]->GetMean();
        double AverageLPMTQE = Sta_LPMTQE[FileId]->GetMean();
        DataWithError AverageSPMTDCR;
        AverageSPMTDCR.Data = Sta_SPMTDCR[FileId]->GetMean();
        AverageSPMTDCR.Error = 0;
        DataWithError P0Mu = GetP0Mu(Sta_SPMTQE[FileId]->GetN(),SPMT0[FileId]);
        // DataWithError TrueMu = GetTrueCharge(RoughMu,AverageSPMTDCR);
        DataWithError TrueMu;
        TrueMu.Data = Sta_SPMTMu[FileId]->GetMean();
        TrueMu.Error = Sta_SPMTMu[FileId]->GetMeanErr();
        ...
        cout<<"Q"<<FileId<<" finished"<<endl;
        // }
    }

    TFile *outputfile = TFile::Open("/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/Alter_10Percent_100PE_B12/npeLPMTtonpeSPMT_B12_DetSim_AllPMT_DetCenter_"+ChosenTypeString+".root","RECREATE");

    for (int FileId = 0; FileId < 21; FileId++)
    {
        outputfile->mkdir(TString::Format("%1d",FileId));
        outputfile->cd(TString::Format("%1d",FileId));
        Hist_TrueLPMTMu_QE[FileId]->Write();
        Hist_TrueLPMTMu[FileId]->Write();
        Hist_SPMTDCR[FileId]->Write();
        Hist_SPMTQE[FileId]->Write();
        Hist_LPMTQE[FileId]->Write();
        Hist_SPMTMu[FileId]->Write();
        Hist_SPMTMu_QE[FileId]->Write();
        outputfile->cd();
    }
