...
{
    TFile *inputfile = TFile::Open("/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/Alter_10Percent_100PE/npeLPMTtonpeSPMT_test.root","read");
    ...
    for (int FileNum = 0; FileNum<21; FileNum++)
    {
        ...
        TotalNonlinearity[FileNum]=result.Data*100.;
        TotalNonlinearityError[FileNum]=result.Error*100.;
    ...
    TPad* pad1 = new TPad("pad1", "pad1", 0.00, 0.32, 1.00, 1.00);
    TPad* pad2 = new TPad("pad1", "pad1", 0.00, 0, 1.00, 0.32);
    pad1->SetMargin(0.12, 0.1, 0.0, 0.1);
    ...
    Below1PE->SetLineColor(1);
    Below1PE->SetLineStyle(2);

    // CalibToTrue->GetXaxis()->SetRangeUser(70000,3000000);

    CalibToTrue->GetXaxis()->SetTitle("Total PE [p.e.]");
    CalibToTrue->GetYaxis()->SetTitle("Reconstructed total PE [p.e.]");
    ...
    RecToTrue->SetMarkerColor(2);
    RecToTrue->SetLineColor(2);
    RecToTrue->SetMarkerSize(1.0);
    RecToTrue->SetMarkerStyle(21);
    RecToTrue->SetLineWidth(3);

    TLegend *legenda =new TLegend(0.4684814,0.02990608,0.9226361,0.2801532);
    legenda->SetTextFont(72);
    legenda->SetTextSize(0.08341572);
    legenda->AddEntry(CalibToTrue,"After calibration","pe");
    legenda->AddEntry(RecToTrue,"Before calibration","pe");
    ...
    CalibRatioToTrue->GetXaxis()->SetTitle("Total PE [p.e.]");
    ...
    CalibRatioToTrue->GetXaxis()->SetTitleOffset(0.77);
    CalibRatioToTrue->GetYaxis()->SetTitleOffset(0.35);
    CalibRatioToTrue->SetMinimum(0.99);
    CalibRatioToTrue->SetMaximum(1.012);
    CalibRatioToTrue->GetYaxis()->SetNdivisions(505);
