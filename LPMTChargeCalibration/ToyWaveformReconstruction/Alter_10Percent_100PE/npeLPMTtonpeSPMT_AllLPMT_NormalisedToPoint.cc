...
int npeLPMTtonpeSPMT_AllLPMT_NormalisedToPoint()
{
    // 初始化要用到的PMT参数
    double LPMTQE[17612];
    double SPMTQE[25600];
    ...
    // 定义输出文件及输出格式
    TFile *outputfile = TFile::Open("/junofs/users/chuziliang125/LPMTChargeCalibration/ToyWaveformReconstruction/Alter_10Percent_100PE/npeLPMTtonpeSPMT_NormalisedToPoint.root","RECREATE");
    ...
    for (LPMTID = 0; LPMTID < 17612; LPMTID++)
    {
        ...
        for (int i = 0; i < 21; i++) // 去掉不正常的点
        {
            ...
            if (i==9)
            {
                ChooseLPMTPoint.Data  = npeLPMTtonpeSPMT_tot->GetPointY(i);
                ChooseLPMTPoint.Error = npeLPMTtonpeSPMT_tot->GetErrorY(i);
                ChooseSPMTPoint.Data  = npeLPMTtonpeSPMT_tot->GetPointX(i);
                ChooseSPMTPoint.Error = npeLPMTtonpeSPMT_tot->GetErrorX(i);
            }
        }
        // f1->SetLineColor(2);
        ...
        for (int i = 0; i < 21; i++) // 计算Nonlinearity和Error
        {
            ...
            // 采用归一化到一个点的方法计算非线性
            Nonlinearity[i] = (npeLPMTtonpeSPMT_tot->GetPointY(i)/ChooseLPMTPoint.Data)/(npeLPMTtonpeSPMT_tot->GetPointX(i)/ChooseSPMTPoint.Data)-1;
            double GetError = 0;
