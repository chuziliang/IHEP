...
int npeLPMTtonpeSPMT_BinnedByR()
{
    int SPMTID[6];
    ...
    for (int num = 0; num < 227; num++)
    {
        ...
        string Edep_RName = "Edep_R spectrum" + subname;
        TStatistic *Edep_Rstatistic = new TStatistic((TString)Edep_RName);
        ...
        for (int i = 0; i < charge->GetEntries(); i++)
        {
            ...
            Edep_Rstatistic->Fill(sqrt(Edep_x*Edep_x + Edep_y*Edep_y + Edep_z*Edep_z));
        }
        ...
        double R = Edep_Rstatistic->GetMean();
        double R_error = Edep_Rstatistic->GetRMS();
        for (int id = 0; id < 6; id++)
        {
        ...
        if (TrueThetaMean < 40)
        {
            ...
            TotR.push_back(R/1000);
            TotRError.push_back(R_error/1000);
        }
        
        Edep_Rstatistic->Delete();
    ...
    TGraphErrors *Factor_positions = new TGraphErrors(TotFactor.size(),
                                                            &TotR[0],&TotFactor[0],
                                                            &TotRError[0],&TotFactorError[0]);

    TCanvas *c2 = new TCanvas("c2","c2",1);
    Factor_positions->SetFillStyle(3002);
    Factor_positions->SetMarkerSize(1.0);
    Factor_positions->SetMarkerStyle(20);
    Factor_positions->SetTitle("Calibration factor at different positions");
    Factor_positions->GetXaxis()->SetTitle("R/m");
    Factor_positions->GetYaxis()->SetTitle("Calibration factor");

    // Factor_positions->Draw("a");
    Factor_positions->Draw("a P");

    TLine *l = new TLine(0,TrueFactor.Data,18,TrueFactor.Data);
    l->Draw("same");
    TLine *lh = new TLine(0,TrueFactor.Data+TrueFactor.Error,18,TrueFactor.Data+TrueFactor.Error);
    lh->SetLineStyle(2);
    lh->Draw("same");
    TLine *ll = new TLine(0,TrueFactor.Data-TrueFactor.Error,18,TrueFactor.Data-TrueFactor.Error);
    ll->SetLineStyle(2);
    ll->Draw("same");
