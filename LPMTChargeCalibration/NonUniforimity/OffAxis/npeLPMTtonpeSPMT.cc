...
void npeLPMTtonpeSPMT(int num, int DegreeNum)
{
    TFile *inputfile = TFile::Open(TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/NonUniforimity/OffAxis/ChargeSpectrum/ChargeSpectrum%.1fMeV%.1fDEG.root",Energy[num],Degree[DegreeNum]),"read");
    ...
    {
        TString LPMTnpeSpectrumName = TString::Format("LPMT charge spectrun %.1fMeV %.1fm %.1fDEG",Energy[num],position[i]*R,Degree[DegreeNum]);
    ...
    TFile *output = new TFile(TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/NonUniforimity/OffAxis/npeLPMTtonpeSPMTroot/npeLPMTtonpeSPMT%.1fMeV%.1fDEG.root",Energy[num],Degree[DegreeNum]),"RECREATE");
