...
void DrawNonUniforimity(int DegreeNum)
{
    ...
    {
        TFile *inputfile = TFile::Open(TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/NonUniforimity/OffAxis/npeLPMTtonpeSPMTroot/npeLPMTtonpeSPMT%.1fMeV%.1fDEG.root",Energy[num],Degree[DegreeNum]),"read");
    ...
    TFile *TrueNonlinearityFile = TFile::Open(TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/NonUniforimity/OffAxis/TrueNonlinearity/npeLPMTtonpeSPMTroot/npeLPMTtonpeSPMT%.1fDEG.root",Degree[DegreeNum]),"read");
    TGraphErrors *TrueNonlinearityGraph[6];
    TF1 *TrueNonlinearityLine[6];
    for (int PMTID = 0; PMTID < 6; PMTID++)
    {
        TrueNonlinearityGraph[PMTID] = (TGraphErrors*)TrueNonlinearityFile->Get(TString::Format("LPMT npe to SPMT_%1d npe",PMTID));
        TrueNonlinearityLine[PMTID] = TrueNonlinearityGraph[PMTID]->GetFunction("pol1");
        TrueNonlinearityLine[PMTID]->SetLineStyle(2);
    }
    TrueNonlinearityFile->Close();
    
    TFile *outputfile = TFile::Open(TString::Format("/junofs/users/chuziliang125/LPMTChargeCalibration/NonUniforimity/OffAxis/NonUniforimity%.1fDEG.root",Degree[DegreeNum]),"recreate");
    ...
    for (int PMTID = 0; PMTID < 6; PMTID++)
    {
        TString mgName = TString::Format("LPMT npe to SPMT_%1d npe %.1fDEG off-axis",PMTID,Degree[DegreeNum]);
        ...
        TrueNonlinearityLine[PMTID]->Draw("same");
        legenda->AddEntry(TrueNonlinearityLine[PMTID],"Detector center events");
