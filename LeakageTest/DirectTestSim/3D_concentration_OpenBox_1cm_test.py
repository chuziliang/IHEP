...
TotalTime = 100     #一共多少秒
tbin = 100000          #时间共多少bin
LeakageRate = 1.0E-9    #Pa*m3/s
LengthX = 0.03      #大约是dr的20倍
LengthY = 0.03
LengthZ = 0.03
dr = 0.0005
...
for ti in range(tbin): 
    ...
    # InceraseX[0,:,:] = 2*InceraseX[1,:,:] -InceraseX[2,:,:]
    # InceraseX[nx-1,:,:] = 2*InceraseX[nx-2,:,:] - InceraseX[nx-3,:,:]

    # InceraseY[:,0,:] = 2*InceraseX[:,1,:] -InceraseX[:,2,:]
    # InceraseY[:,ny-1,:] = 2*InceraseX[:,ny-2,:] -InceraseX[:,ny-3,:]

    # InceraseZ[:,:,0] = 2*InceraseX[:,:,1] - InceraseX[:,:,2]
    # InceraseZ[:,:,nz-1] = 2*InceraseX[:,:,nz-2] - InceraseX[:,:,nz-3]
...
filename = 'Result/OpenBox_1cm_test2.npz'
...
filename = 'Result/OpenBox_1cm_test2.pdf'
