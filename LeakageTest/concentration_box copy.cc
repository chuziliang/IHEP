...
void concentration_box()
{
    ...
    for (int ti = 0; ti < tbin; ti++)
    {
        ...
        {
            TotalSF6_test += Ci[i]*0.1*(4.0*TMath::Pi()/3.0)*(pow(dh*(i+1),3)-pow(dh*(i),3));//这里需要将PPM转换为Pa*m3
        }
        TotalSF6Totime.push_back(TotalSF6_test);
        Time.push_back(dt*(ti+1));
        SF6IncreaseRatio.push_back(TotalSF6_test/(dt*(ti+1)));
    }
