...
int main(int argc,char** argv) {
    ...
    cout<<"Total file number : "<<fileNum<<endl;

    string SubRunName;//定义runName中有用的重要信息，如时间等，在输出文件名中保留这些信息
    SubRunName = runName[fileNum-1].substr(runName[fileNum-1].length()-25,21);
    TFile *ofile = new TFile(("./result/"+SubRunName+markedF+".root").c_str(),"recreate");
    ...
    TGraph *DataRate = new TGraph(128);
    DataRate->SetTitle("Data Rate");
    DataRate->GetXaxis()->SetTitle("Channel Number");
    DataRate->SetMarkerStyle(8);
    DataRate->SetMarkerSize(0.7);
    ...
    ofile->mkdir("FineTime");
    ofile->cd("CoarseTime");
    for (int i = 0; i<128; i++)
    {
        CTminusTS[i] = new TH1F(TString::Format("CT - Time stamp Chn%1d",i),TString::Format("CT - Time stamp Chn%1d",i),1000,-1000,0);
        CTminusTS[i]->GetXaxis()->SetTitle("CoarseTime - TimeStamp");
    }
    ofile->cd("FineTime");
    for (int i = 0; i<128; i++)
    {
        FThist[i] = new TH1F(TString::Format("Fine Time Chn%1d",i),TString::Format("Fine Time Chn%1d",i),1024,0,1024);
        FThist[i]->GetXaxis()->SetTitle("FineTime");
    }
    ...
    for (int lpFile=0; lpFile<fileNum; lpFile++) {
        ...
        ifstream myFile ((rawDataPath+runName[lpFile]).c_str(), ios::in | ios::binary); //remove FileName for DAVIS
	if (!myFile)
	{
	...
	else
	{
        ...
        cout<<"This file number : "<<lpFile+1<<endl;
        cout<<"test path: "<<(rawDataPath+runName[lpFile]).c_str()<<endl;
        ///////////////////////////////////
                    ...
                    for (int loopN = 0; loopN<Header_NumOfRChn; loopN++) {
                        ...
                        //if (thisCT>0) {
                            ...
                            if (iChnNo<128)   {
                                ...
                                ofile->cd();
                                ofile->cd("CoarseTime");
                                CTminusTS[iChnNo]->Fill(thisCT - (int)Header_EvtTimeStmp);
                                ofile->cd("FineTime");
                                FThist[iChnNo]->Fill(thisFT);
                                ...
                                ofile->cd("CoarseTime");
                                //CoarseTime[thisMode][iChnNo]->AddPoint(loopTotalN,thisCT);
                                CTminusTS[iChnNo-128]->Fill(thisCT - (int)Header_EvtTimeStmp);
                                ofile->cd("FineTime");
                                FThist[iChnNo-128]->Fill(thisFT);
                            }
    ...
    for (int i = 0; i < 128; i++)
    {
        DataRate->SetPoint(i,i,CTminusTS[i]->GetEntries()/60.0);
    }
    ...
    c2->Print(("./result/"+SubRunName+markedF+".pdf").c_str(),"pdf");
    ...
    c5->cd();
    DataRate->Draw("AP");
    c5->Print(("./result/"+SubRunName+markedF+".pdf").c_str(),"pdf");
    ofile->cd();
    c5->Write();
    ...
    for (int iSign = 0; iSign<=1; iSign++) {    // ping:0, pong:1
        for (int iChn=0; iChn<128; iChn++) {
            ...
            if (iChn%16 == 15)
            {
                c3->Print(("./result/"+SubRunName+markedF+".pdf").c_str(),"pdf");
                ofile->cd();
                ofile->cd("canvas");
                c3->Write();
            }
    ...
    for (int iChn=0; iChn<128; iChn++) {
        ...
        if (iChn%16 == 15)
        {
            c4->Print(("./result/"+SubRunName+markedF+".pdf").c_str(),"pdf");
            // ofile->cd();
            // ofile->cd("canvas");
            //c3->Write();
        }
