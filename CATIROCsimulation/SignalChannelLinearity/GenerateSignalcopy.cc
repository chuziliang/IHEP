void GenerateSignalcopy()
{
    ...
    for (Int_t PEnum = 1; PEnum <= 10; PEnum++)
    {
        ...
        //for (int j = 0; j < 100 ; j++)
        {
            tmp[PEnum-1] = new TH1F(TString::Format("signal of %1d PE",PEnum),TString::Format("signal of %1d PE",PEnum),1008,0,8+TimeWindow);
            for ( Int_t i = 0; i<PEnum; i++)
            {
                ...
                tmp[PEnum-1]->Add(func);
            }

            TH1 *hr = 0;
            TH1 *hi = 0;
            hr = tmp[PEnum-1]->FFT(hr,"RE");
            hi = tmp[PEnum-1]->FFT(hi,"IM");
