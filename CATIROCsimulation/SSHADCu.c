void SSHADCu()
{
	...
	TString name;
	name = "./ch1.txt";
	TFile *ofile = new TFile("hist_in_different_trigger.root","recreate");
	TH1 *result2[16];
	for (int j = 0; j<16 ; j++)
	{
		result2[j] = new TH1F(TString::Format("charge spectrum of %1d DACu",200+50*j),TString::Format("charge spectrum of %1d DACu",200+50*j),1000,0.0,300);
	}
	ifstream in(name);
	while (in.good()){
		double ori, nan;
