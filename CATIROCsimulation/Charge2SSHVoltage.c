void Charge2SSHVoltage()
{
	...
	//double cha[17973];
	//double DACu[17973];
	TH2 *result = new TH2D("Charge-SSH","Charge-SSH",1000,0,800,1000,0.0,2.0); 
	...
	while (in.good()){
		...
		if ( RMS < 0.1 )
		{
			...
			for(Int_t i=0;i<n;i++)
			{
				...
				response = complex<double>(0.0,TMath::Pi()*i*RC/1008.0)*RC/(Cf*complex<double>(1.0,2.0*TMath::Pi()*i*RC/1008.0)*complex<double>(1.0,2.0*TMath::Pi()*i*RC/1008.0)*complex<double>(1.0,2.0*TMath::Pi()*i*RC/1008.0));
			...
			result->Fill(maximum,charge);
			result2->Fill(maximum,signalmax);
			delete hb;
			delete fft_back;
		}
		delete tmp;
	...
	result->GetXaxis()->SetTitle("SSH maximum/mV");
