void Charge2SSHADCucopy()
{
	...
	TH1F *result2 = new TH1F("ChargeSSH","ChargeSSH",1000,0.0,800);
	TH1F *result3 = new TH1F("Charge trigger","Charge trigger",100,0.0,2.0);
	TH2 *Charge2DACu = new TH2D("Charge-SSH","Charge-SSH",1000,0,800,1000,0.0,2); 
	ifstream in(name);
	while (in.good()){
		double ori, nan;
		...
		for (int i = 0; i<n; i++)
		{
			...
			{
				ChargeSSH = (hbSSH->GetBinContent(i+DelayTime));//*0.3854+78.59;
	...
	TF1* func = new TF1("f1","[0]*x+[1]",100,400);
	func->SetParNames("k","b");
	Charge2DACu->Fit("f1","RL");
	func->Draw("SAME");
