...
double* extractor::extract() {
	...
	double max;
	for (int i = 0; i < 128; i ++) {
		tmpHist = (TH1F*)myFile->Get(Form("/HighGain/Ping_Chn%d_HG", i));
		...
		tmpHist = (TH1F*)myFile->Get(Form("/HighGain/Pong_Chn%d_HG", i));
		tmpHist->Fit(gaus, "R");
		data[i * 8 + 2] = gaus->GetParameter(1);
		data[i * 8 + 3] = gaus->GetParameter(2);

		tmpHist = (TH1F*)myFile->Get(Form("/LowGain/Ping_Chn%d_LG", i));
		data[i * 8 + 4] = tmpHist->GetMean();
		data[i * 8 + 5] = tmpHist->GetRMS();
		tmpHist = (TH1F*)myFile->Get(Form("/LowGain/Pong_Chn%d_LG", i));
