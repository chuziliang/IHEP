...
int main(int argc,char** argv)
{
	...
	ifstream file_ref_1;
  	file_ref_1.open("/junofs/users/chuziliang125/PED_analysis20230816/Analysis/input_binary_filename.txt");
	
	string filename;
	string OutputDirectory;
	int target_line;

	OutputDirectory="/junofs/users/chuziliang125/PED_analysis20230816/results/";
	...
	//while (!file_ref_1.eof())
	{
    	...
    	while (getline(file_ref_1, filename)) 
		{
			...
			{
				line = "/junofs/users/chuziliang125/PED_analysis20230816/data/"+filename;
				break; // 已经获取到目标行，跳出循环
			}
        current_line++; // 更新当前行号
    	}
